...
├── _nodes/
│   ├── index.ts               ← 日本語・個人向けエントリ（Node.js 実行用）
│   ├── index.flyde.ts         ← 日本語・個人向けエントリ（Flyde エディタ用）
│   ├── index.en_US.ts         ← 英語・個人向けエントリ（Node.js 実行用）
│   ├── index.en_US.flyde.ts   ← 英語・個人向けエントリ（Flyde エディタ用）
...
export const コマンド実行 = localizeNode(core.RunCommand, i18n.RunCommand)
```
...
| ファイル | 言語 | 用途 |
|---|---|---|
| `index.ts` / `index.flyde.ts` | 日本語 | デフォルト |
| `index.en_US.ts` / `index.en_US.flyde.ts` | 英語 | 英語圏ユーザー |

`.flyde` ファイルの `source.data` パスで言語を切り替える。
...
`_core/` には全ノードを実装し、エントリポイント（`index.ts`）が全ノードを export する。
...
3. `_nodes/index.ts` / `index.en_US.ts` に `localizeNode()` 呼び出しを追加する（`.flyde.ts` は再エクスポートのため変更不要）
